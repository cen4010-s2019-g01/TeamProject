...
Latest File Update: 2018-Sep-30
*/
...
$statsTop=' . ';
for($i=0;$i<$tKey;$i++) $statsTop.=($i<>$top?'<a href="'.$main_url.'/'.$indexphp.'action=stats&amp;top='.$i.'&amp;days='.$days.'&amp;lst='.$lst.'" class="mnblnk">'.$l_stats_top.' '.fTopa($i).'</a> . ':$l_stats_top.' '.fTopa($i).' . ');
$makeLim=fTopa($top);
...
for($i=0;$i<=$lstLim;$i++){
if($i!=$lst and $statsOptL[$i]!='') $statsOpt.=$insbr.'<a href="'.$main_url.'/'.$indexphp.'action=stats&amp;top='.$top.'&amp;days='.$days.'&amp;lst='.$i.'" class="mnblnk">'.$statsOptL[$i].'</a>';
...
if($stats_barWidth>$stats_barWidthLim) $key='<a href="'.gen_vthread_url($cols[2], $forumNames[$cols[2]][0], $cols[0], $cols[1], PAGE1_OFFSET+1).'" class="mnblnk">'.$cols[1].'</a>';
else{
$key2='<a href="'.gen_vthread_url($cols[2], $forumNames[$cols[2]][0], $cols[0], $cols[1], PAGE1_OFFSET+1).'" class="mnblnk">'.$cols[1].'</a>';
$key='<a href="'.gen_vthread_url($cols[2], $forumNames[$cols[2]][0], $cols[0], $cols[1], PAGE1_OFFSET+1).'" class="mnblnk">...</a>';
}
...
if(!isset($vMax)) $vMax=$val;
if ($vMax!=0) $stats_barWidth=round(100*($val/$vMax));
if($stats_barWidth>$stats_barWidthLim) $key='<a href="'.$main_url.'/'.$indexphp.'action=userinfo&amp;user='.$cols[0].'" class="mnblnk">'.$cols[1].'</a>';
else{
$key2='<a href="'.$main_url.'/'.$indexphp.'action=userinfo&amp;user='.$cols[0].'" class="mnblnk">'.$cols[1].'</a>';
$key='<a href="'.$main_url.'/'.$indexphp.'action=userinfo&amp;user='.$cols[0].'" class="mnblnk">...</a>';
}
...
if($stats_barWidth>$stats_barWidthLim) $key='<a href="'.gen_vthread_url($cols[3], $forumNames[$cols[3]][0], $cols[0], $cols[2], PAGE1_OFFSET+1).'" class="mnblnk">'.$cols[2].'</a>';
else{
$key2='<a href="'.gen_vthread_url($cols[3], $forumNames[$cols[3]][0], $cols[0], $cols[2], PAGE1_OFFSET+1).'" class="mnblnk">'.$cols[2].'</a>';
$key='<a href="'.gen_vthread_url($cols[3], $forumNames[$cols[3]][0], $cols[0], $cols[2], PAGE1_OFFSET+1).'" class="mnblnk">...</a>';
}
...
if($stats_barWidth>$stats_barWidthLim) $key='<a href="'.$fUrl.'" class="mnblnk">'.$fName.'</a>';
else{
$key2='<a href="'.$fUrl.'" class="mnblnk">'.$fName.'</a>';
$key='<a href="'.$fUrl.'" class="mnblnk">...</a>';
}
...
$subTitle='';
$subTitles=array($l_stats_popular, $l_stats_viewed, $l_stats_aUsers, $l_stats_pop_forums);
$subTop=fTopa($top);

if($lst==0 and $top==0){}
else{
if(isset($subTitles[$lst])){
$subTitle.=' - '.$subTitles[$lst];
if( ($top>0) ) $subTitle.=' - '.$l_stats_top.' '.$subTop;
}
}

$title=$title.$l_stats.$subTitle;
