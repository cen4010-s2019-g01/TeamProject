...
Copyright (C) Paul Puzyrev www.minibb.com
Latest File Update: 2019-Oct-14
*/
...
function sendMail($email, $subject, $msg, $from_email, $errors_email, $reply_to_email='') {
// Function sends mail with return-path */

if (!isset($GLOBALS['genEmailDisable']) or $GLOBALS['genEmailDisable']!=1){

if($reply_to_email=='') $reply_to_email=$from_email;
...
if(substr_count($reply_to_email,"\n")>0) $reply_to_email=$GLOBALS['reply_to_email'];
...
$from_email="From: {$GLOBALS['emailname']} <{$from_email}>{$eeol}Reply-To: {$reply_to_email}{$eeol}Errors-To: {$errors_email}{$eeol}Return-Path: {$errors_email}{$eeol}MIME-Version: 1.0{$eeol}Content-Type: text/plain{$charset}{$eeol}";
...
<a href="http://www.minibb.com/" target="_blank" class="mnblnk mnblnkn"><img src="{$GLOBALS['main_url']}/img/minibb.png" alt="{$GLOBALS['sitename']} {$GLOBALS['l_poweredBy']} {$software} miniBB &reg;" title="{$GLOBALS['sitename']} {$GLOBALS['l_poweredBy']} {$software} miniBB &reg;" class="vmiddle" /></a>
out;
