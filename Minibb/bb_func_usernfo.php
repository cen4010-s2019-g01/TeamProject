...
if ($val!='' and $GLOBALS['user_id']>0) return '<a href="'.$val.'" target="_blank" rel="nofollow" class="mnblnk">'.wrapText($GLOBALS['post_word_maxlength'], $val).'</a>';
else {
$vali=addslashes($val);
$val="<script type=\"text/javascript\">document.write('<'+'a'+' h'+'ref'+'='+'\"'+'{$val}'+'\" target=\"_blank\" rel=\"nofollow\" class=\"mnblnk\">'+'{$vali}'+'<'+'/'+'a'+'>');</script>";
}
...
$newVal="{$GLOBALS['l_totalTP']}: ".parseStatsNum($coreVal)." / {$GLOBALS['l_liveTP']}: ".parseStatsNum($liveVal)." / <a href=\"{$GLOBALS['main_url']}/{$GLOBALS['indexphp']}action=archives&amp;user={$GLOBALS['user']}\" class=\"mnblnk\">{$GLOBALS['l_archivedTP']}</a>: ".parseStatsNum($arcVal);
}
}
else{
...
$newVal="{$GLOBALS['l_totalTP']}: ".parseStatsNum($val)." / {$GLOBALS['l_liveTP']}: ".parseStatsNum($liveVal)." / <a href=\"{$GLOBALS['main_url']}/{$GLOBALS['indexphp']}action=archives&amp;user={$GLOBALS['user']}\" class=\"mnblnk\">{$GLOBALS['l_archivedTP']}</a>: ".parseStatsNum($arcVal);
}
}
else{
...
$whatValue.="<li><a href=\"{$urlp}\" class=\"mnblnk\">{$lastT[2]}</a></li>";
}
...
if(file_exists($pathToFiles.'msg.php')){
$linkParse="{$main_url}/msg.php?id=POST_ID";
}
else{
$linkParse="{$main_url}/{$indexphp}action=search&amp;loc=1&amp;forum=FORUM_ID&amp;topic=TOPIC_ID&amp;page=POST_ID";
}

do {
$topicVals[$lastT[0]]="<li><a href=\"".str_replace(array('FORUM_ID', 'TOPIC_ID', 'POST_ID'), array($lastT[1], $lastT[0], $postsAll[$lastT[0]]), $linkParse)."\" class=\"mnblnk\" rel=\"nofollow\">{$lastT[2]}</a></li>";
}
while ($lastT=db_simpleSelect(1));
}
...
if($stats_barWidth>$stats_barWidthLim) $key="<a href=\"{$urlp}\" class=\"mnblnk\">{$forumNames[$k]}</a>";
else{
$key2="<a href=\"{$urlp}\" class=\"mnblnk\">{$forumNames[$k]}</a>";
$key="<a href=\"{$urlp}\" class=\"mnblnk\">...</a>";
}
